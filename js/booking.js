...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Duration limits - set to minutes with max 60
    durationInput.min = 1;
    durationInput.max = 60;
    durationInput.placeholder = 'Minutes (1-60)';
    ...
    bookingForm.addEventListener('submit', async (e) => {
        ...
        if (bookingData.duration < min || bookingData.duration > max) {
            alert(`Duration must be between ${min} and ${max} minutes`);
            return;
        }
        ...
        try {
            ...
            // Calculate cost (₹50 per hour, prorated for minutes)
            const minutes = (endTime - startTime) / (1000 * 60);
            const totalCost = (minutes / 60) * 50;
            ...
            if (bookingError) {
                if (bookingError.code === '23P01') {
                    alert('Spot unavailable for the chosen time. Please adjust your booking.');
                } else {
                    throw bookingError;
                }
                bookBtn.textContent='Book Spot'; 
